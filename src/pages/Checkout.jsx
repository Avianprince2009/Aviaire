...
// Inline API helper (avoids depending on apiClient filename/path issues)
const postJson = async (path, payload) => {
  const baseUrl = import.meta.env.VITE_API_BASE_URL || import.meta.env.VITE_API_URL || ''
  const url = `${baseUrl}/api/v1/${path}`.replace(/\/+/, '/').replace(':/', '://')
...
const Checkout = ({ cart = [] }) => {
  ...
  const [placingPayment, setPlacingPayment] = useState(false)
  ...
  const userEmail = authStore?.user?.email || form.email
  ...
  const validate = () => {
    ...
    for (const key of required) {
      if (!String(check[key] || '').trim()) {
        return `Missing ${key.replace(/([A-Z])/g, ' $1').toLowerCase()}`
      }
    }

    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(check.email)) return 'Invalid email'

    return null
  }

  const handleVerificationAndSaveOrder = async (reference) => {
    setPlacingPayment(true)
    setError(null)

    try {
      const payload = {
        reference,
        // backend expects shipping fields + email
      ...
      const resp = await postJson('paystack/verify', payload)
      ...
      // optional: go to a success page if you have one; keeping existing flow
      // navigate('/checkout-success')
    } catch (err) {
      setError(err?.message || 'Payment verification failed. Please try again.')
    } finally {
      setPlacingPayment(false)
    }
  }

  const paystackConfig = {
    reference: `AV-${Date.now()}`,
    email: userEmail,
    amount: amountKobo,
    publicKey: import.meta.env.VITE_PAYSTACK_PUBLIC_KEY || import.meta.env.VITE_PAYSTACK_PUBLIC,
    currency: 'NGN',
    // Paystack will call these callbacks
    onSuccess: async (paymentData) => {
      ...
      await handleVerificationAndSaveOrder(reference)
    },
    onCancel: () => {
  ...
  const initiatePaystack = (e) => {
    ...
    // disable button while popup is open + verify runs
    setPlacingPayment(true)

    // Ensure paystackConfig.reference matches current click
    const reference = `AV-${Date.now()}-${Math.random().toString(16).slice(2, 8).toUpperCase()}`
          ...
          <div className="bg-zinc-900/40 backdrop-blur-xl border border-[#c9a961]/15 p-6 md:p-8 rounded-xl">
            ...
            <div className="mt-6 pt-6 border-t border-[#c9a961]/10">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                <p className="text-sm text-zinc-400">Placed: {new Date(orderSuccess.placedAt).toLocaleString()}</p>
  ...
  return (
    <div className="min-h-screen bg-[#111111] text-white pt-24 pb-16 px-6 md:px-12">
      <div className="max-w-5xl mx-auto">
        <h1 className="text-4xl md:text-5xl font-serif font-light text-[#C9A961] text-center tracking-wide mb-4">Checkout</h1>
        <p className="max-w-2xl mx-auto mb-10 text-sm text-center text-zinc-400 md:text-base">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-zinc-900/40 backdrop-blur-xl border border-[#c9a961]/15 p-6 md:p-8 rounded-xl">
            ...
            <form onSubmit={initiatePaystack} className="space-y-4">
              ...
              >
                ...
                {placingPayment ? 'Processing payment...' : `Place Order (${total.toLocaleString()})`}
              </button>
          ...
          <div className="bg-zinc-900/40 backdrop-blur-xl border border-[#c9a961]/15 p-6 md:p-8 rounded-xl">
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="text-xs tracking-wider uppercase text-zinc-500 mt-1">{item.collection}</p>
                      </div>
            ...
            <div className="mt-8 pt-6 border-t border-[#c9a961]/10">
              ...
              <p className="mt-4 text-xs text-zinc-600">Payment will be processed by Paystack.</p>
            </div>
          </div>
        </div>
      </div>
